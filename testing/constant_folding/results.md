Here's a **comprehensive test program** for `constant_folding` (global constant propagation):

```asm
; ===================================================================
; TEST: constant_folding - Global Constant Propagation
; Run with: ./v32opt test_constant_folding.asm -O2 -v
; Note: Requires -O2 to enable constant_folding (global data-flow)
; ===================================================================
...
|------|----------------|--------|
| Scenario 1 | `MOV R2, 0x2A` | Simple propagation |
| Scenario 2 | `MOV R2, 0xA` + `MOV R3, 0xA` + `MOV R4, 0xA` | Chain propagation |
| Scenario 3 | `MOV R2, 0x8` | Arithmetic result propagated |
| Scenario 4 | `MOV R2, 0x1` | Propagates through JT |
| Scenario 5 | `MOV R3, 0xA` + `MOV R4, 0x14` | Multiple constants |
| Scenario 6 | **Unchanged** | CALL invalidates R1 |
| Scenario 7 | `MOV R2, 0x64` | Across blocks |
| Scenario 8 | `MOV R2, 0x5` + `MOV R3, 0x8` + `MOV R4, 0x6` | Complex propagation |
| Scenario 9 | **Unchanged** | Non-constant source |
| Scenario 10 | `MOV R2, 0xA` | Loop with constant |
| Scenario 11 | `MOV R4, 0x0` | Conditional with constants |
| Scenario 12 | `MOV R2, 0x0` + `MOV R4, 0xFFFFFFFF` (or similar) | Zero and negative |
| Scenario 13 | `MOV R2, 0x1000000` | Large constants |
| Scenario 14 | `MOV R3, 0xA` + `MOV R4, R2` unchanged | Mixed constants/vars |

---
### **How to Test**
1. Save as `test_constant_folding.asm`
2. Run: `./v32opt test_constant_folding.asm -O2 -v`
   - Note: Requires `-O2` to enable `constant_folding` (global data-flow phase)
3. Verify each function matches expected output
4. Confirm constants are correctly propagated and folded

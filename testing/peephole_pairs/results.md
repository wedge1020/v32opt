Here's a **comprehensive test program** for `peephole_pairs` covering all optimization patterns:
...
|------|----------------|--------|
| `test_ieq_cib` | IEQ only (CIB removed) | IEQ + CIB redundancy |
| `test_ine_cib` | INE only (CIB removed) | INE + CIB redundancy |
| `test_ieq_cib_multiple` | Both CIBs removed | Multiple IEQ + CIB |
| `test_ieq_cib_jt` | **Unchanged** | CIB preserved before JT |
| `test_ieq_cib_jf` | **Unchanged** | CIB preserved before JF |
| `test_ine_cib_jt` | **Unchanged** | CIB preserved before JT |
| `test_bnot_pair` | Both commented out | Self-inverting |
| `test_ineg_pair` | Both commented out | Self-inverting |
| `test_not_pair` | Both commented out | Self-inverting |
| `test_bnot_diff` | **Unchanged** | Different registers |
| `test_xor_reg_pair` | Both commented out | XOR pair |
| `test_xor_imm_pair` | Both commented out | XOR pair |
| `test_xor_neg_pair` | Both commented out | XOR pair |
| `test_xor_diff_reg` | **Unchanged** | Different destination |
| `test_xor_diff_imm` | **Unchanged** | Different immediate |
| `test_push_pop` | Both removed | PUSH/POP pair |
| `test_push_pop_comments` | Both removed | Through comments |
| `test_push_pop_multiple` | Both pairs removed | Multiple PUSH/POP |
| `test_push_pop_diff` | **Unchanged** | Different registers |
| `test_combined` | All pairs optimized | Combined patterns |

---
### **How to Test**
1. Save as `test_pairs.asm`
2. Run: `./v32opt test_pairs.asm -fopt_peephole_pairs -v`
